nhelp = 0
while nhelp != 1:  # Проверка, чтобы ввели число сотрудников. | Диалоговый режим с пользователем и обработкой ошибок ввода
    try:
...
endend = []
k = 0  # Счетчик для проверки
while k < 1:  # Проверка, чтобы вместа числа не ввели что то другое | Диалоговый режим с пользователем и обработкой ошибок ввода
    try:
...
#print(b) Проверка работоспособности
k_1 = 0  # Счетчик для проверки
while k_1 < 1:  # Проверка, чтобы вместа числа не ввели что то другое | Диалоговый режим с пользователем и обработкой ошибок ввода
    try:
        ...
        k_1 += 1
# print(d) Проверка работоспособности

while n > 0: # Счет минимальных затрат: мин тариф * макс расстояния. Цикл проходится по всему списку
    maxik = max(d)  # киллометраж j-сотрудника
    minik = min(b)  # тариф i-сотрудника
...
if 1 > 0:
    ...
    print(end[::2])  # Вывод машина по порядку
    print(sum)
    c = sum  # Вывод суммы - словами
    ...
    if (ts == 1 and ((td or te) != 0)): # Вывод суммы - словами
        list.append("Сто")
